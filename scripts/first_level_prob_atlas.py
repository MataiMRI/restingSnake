...
subject = sys.argv[2]
session = sys.argv[3]
ntwk = sys.argv[4].encode()
mask_file = sys.argv[5]
func_file = sys.argv[6]
confound_file = sys.argv[7]
# task = 
rep_time = float(sys.argv[8])
hp = float(sys.argv[9])
lp = float(sys.argv[10])
fwhm = float(sys.argv[11])
fdr_alpha = float(sys.argv[12])
thresh = float(sys.argv[13])
num_networks = sys.argv[14]
...
print('{mask} \n {func} \n {confounds} \n \n'.format(mask = mask_file, func = func_file, confounds = confound_file) )
