...
parser.add_argument(
    'nifti_output',
    help='Name and location of .nii output of First Level analysis for selected resting-state network',
    type=str
)
...
parser.add_argument(
    '-a_img',
    help='Specify Atlas .nii image file',
    type=str,
    dest='atlas_image'
)
...
#Load atlas to provide resting-state network options
...
network_nodes = image.index_img(atlas_filename, msdl_networks[args.functional_network]) #define nodes
...
logging.info(f'BOLD signal will be standardized, detrended, and cleaned based with a Bandpass filter set to {args.lowpass}-{args.highpass} Hz and a {args.fwhm} mm full-width-half-maximum smoothing kernel \n')
    
#Create mask using user-specified network nodes from atlas
logging.info('Generating mask for network of interest based on seed coordinates in atlas')
...
logging.info(f'Converting functional network data to timeseries with shape: {network_time_series.shape}')
...
logging.info(f'Converting whole brain data to timeseries with shape: {brain_time_series.shape}')
...
logging.info(f"Network-to-voxel correlation shape: (%s, %s)" % network_to_voxel_correlations.shape)
logging.info("Network-to-voxel correlation: min = %.3f; max = %.3f" % (
...
logging.info(f'Saving plot of {args.functional_network} connectivity THRESHOLDED at {args.connectivity_threshold} to {args.plotting_output}')
display = plotting.plot_stat_map(image.index_img(network_to_voxel_correlations_corrected_img, 0),
                                      ...
                                      title=f'{args.functional_network} functional connectivity thresholded at {args.connectivity_threshold}',
                                      display_mode='z',
                                      vmax = 1,
                                      cmap = 'cold_hot',
